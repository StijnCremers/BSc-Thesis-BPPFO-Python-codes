...
def BPPFO_BFD(data, sort_indic, bin_capacity = np.inf):
  ...
  Returns:
      - bins: A list of lists, where each inner list represents a bin and contains the bin number, the bin's remaining space and the items packed in that bin.
  """
    # Start the timer
    start = t.time()
    ...
    RemainCap = sum([bin[1] ** 2 for bin in bins])     
    
    # Calculate the runtime
    runtime = t.time() - start        

    return num_bins, runtime, RemainCap

filename = "C:/Users/Stijn Cremers/OneDrive/TiU documents/Year 3/BSc Thesis/Python implementation/python311/lib/InstancesBPPFO/N1C1W1_CL1_1_3_C.BPPFI"
data = pd.read_csv(filename, sep=" ", header=None, skiprows = 2)
data.columns = ['Weight', 'Fragility']
data_arr = np.array(data)

objValue0, runtime0, RemainCap0 = BPPFO_BFD(data_arr, 0)
objValue1, runtime1, RemainCap1 = BPPFO_BFD(data_arr, 1)
objValue2, runtime2, RemainCap2 = BPPFO_BFD(data_arr, 2)